...
class MyWindow(QMainWindow, form_class):
    ...
    def quit_app(self):
        ...
        self.mybot.sendMessage(self.chat_id, text=QTime.currentTime().toString("hh:mm:ss") + "\n프로그램이 종료되었습니다.\n총 TR 요청 횟수 : %d회\n -----계좌현황-----\n계좌번호: %s\n%s\n -----보유종목-----\n%s" % (self.tr,account_number, myaccount, mystocks))
        ...
        if self.checkBox_2.isChecked():
            ...
            if current_time > market_start_time and self.shutdown_0900 =='false' and self.ts_1_p =='False':
                ...
                self.mybot.sendMessage(self.chat_id,
                                       text=QTime.currentTime().toString("hh:mm:ss") + "장 시작, 매수전략1 시작하겠습니다.")
                self.trading_strategy_1()
            ...
            #15시 40 분 이후 알고리즘 중지 및 10분 뒤 컴퓨터 종료
                ...
                self.mybot.sendMessage(self.chat_id,
                                       text=QTime.currentTime().toString("hh:mm:ss") + "10분 후 컴퓨터를 종료합니다.")
    ...
    def volume_check(self):
        ...
        if len(self.buy_list) == 0:
            ...
            buy_q = (buy_q//len(self.buy_list))//5 #예수금/선정종목갯수/5 : 종목당 매수금액
            self.tr += 1
